...
if ((($mcat=='merienda')AND($nopax > 99)) OR (($mcat=='lunch_and_dinner')AND($nopax > 49)) OR (($mcat=='specialty') AND ($nopax > 29))) {
	include('connect.php');
	$sql = "INSERT INTO reservation (res_id,firstname,address,motif,type_events,type_res,date,pax,contact,venueaddress,type_catering,lastname,status,time,resdate)
	VALUES (:a,:b,:c,:d,:e,:f,:g,:h,:i,:j,:k,:l,:m,:n,:z)";
	// echo $sql;
	$q = $db->prepare($sql);
